...
public class QuartzManager {
  ...
  public void startOrUpdateJob(Cronjob job) throws InstantiationException, IllegalAccessException,
    ...
    if (isValidName(job.getJobClass())) {
      ...
      if (null == trigger) {// There is no task
        ...
        // Create a task
        JobDetail jobDetail =
            JobBuilder.newJob(jobClass).withIdentity(job.getName(), job.getJobGroup()).build();
        ...
        if (job.getJobTrigger().equals(CronjobEnum.JobTrigger.RUN_ONCE)) {
          ...
          if (job.getTriggerStartDate() == null) {
            runOnceTrigger = TriggerBuilder.newTrigger()
                .withIdentity(job.getName(), job.getJobGroup()).startNow().build();
          } else {
            runOnceTrigger =
                TriggerBuilder.newTrigger().withIdentity(job.getName(), job.getJobGroup())
                    .startAt(job.getTriggerStartDate()).build();
          }
          ...
          if (job.getTriggerStartDate() != null) {
            ...
            if (job.getTriggerStartDate().compareTo(new Date()) > 0) {
              trigger = TriggerBuilder.newTrigger().withIdentity(job.getName(), job.getJobGroup())
                  .startAt(job.getTriggerStartDate())
            ...
            //
            // trigger = TriggerBuilder.newTrigger().withIdentity(job.getName(),
            // job.getJobGroup()).startAt(newStartDate.getTime()).withSchedule(scheduleBuilder.withMisfireHandlingInstructionDoNothing())
            // .build();
            // } else {
            // trigger = TriggerBuilder.newTrigger().withIdentity(job.getName(),
            // job.getJobGroup()).startAt(job.getTriggerStartDate()).withSchedule(scheduleBuilder.withMisfireHandlingInstructionDoNothing())
            // .build();
            // }
          } else {
            trigger = TriggerBuilder.newTrigger().withIdentity(job.getName(), job.getJobGroup())
      ...
      LOGGER
          .error("Not able to find cronjob class, please check. Cronjob [Id= " + job.getId() + "]");
