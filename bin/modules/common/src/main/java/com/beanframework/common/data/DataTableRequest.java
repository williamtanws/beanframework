...
public class DataTableRequest {
  ...
  public void prepareDataTableRequest(HttpServletRequest request) {
        ...
        if (skipColumnIndexes.contains(i) == Boolean.FALSE) {
          if (request.getParameter("order[" + i + "][column]") != null) {
            ...
            if (request.getParameter("columns[" + columnIndex + "][data]") != null) {
              ...
              colSpec.setOrderable(
                  Boolean.valueOf(request.getParameter("columns[" + columnIndex + "][orderable]")));
              colSpec.setRegex(Boolean
                  .valueOf(request.getParameter("columns[" + columnIndex + "][search][regex]")));
              colSpec
                  .setSearch(request.getParameter("columns[" + columnIndex + "][search][value]"));
              colSpec.setSearchable(Boolean
                  .valueOf(request.getParameter("columns[" + columnIndex + "][searchable]")));
    ...
    if (this.pageable != Pageable.unpaged()) {
      this.setPageable(PageRequest.of(this.getPaginationRequest().getPageNumber(),
          this.getPaginationRequest().getPageSize(), Sort.by(sortOrders)));
    }
  }
  ...
  public PaginationCriteria getPaginationRequest() {
    ...
    for (DataTableColumnSpecs colSpec : this.getColumns()) {
      if (colSpec.isSearchable()) {
        if (!AppUtil.isObjectEmpty(this.getSearch())
            || !AppUtil.isObjectEmpty(colSpec.getSearch())) {
  ...
  public AuditCriterion getAuditCriterion() {
    ...
    if (isGlobalSearch() && StringUtils.isNotEmpty(getSearch())) {
      for (DataTableColumnSpecs specs : columns) {
        orCriterion = AuditEntity.or(orCriterion, AuditEntity.property(specs.getData())
            .like(CommonSpecification.convertToLikePattern(getSearch())));
      }
    } else {
      for (DataTableColumnSpecs specs : columns) {
        if (StringUtils.isNotBlank(specs.getSearch())) {
          orCriterion = AuditEntity.or(orCriterion, AuditEntity.property(specs.getData())
              .like(CommonSpecification.convertToLikePattern(specs.getSearch())));
        }
      }
    }
  ...
  public String toString() {
    return "DataTableRequest [uniqueId=" + uniqueId + ", start=" + start + ", length=" + length
        + ", search=" + search + ", regex=" + regex + ", columns=" + columns + ", orders=" + orders
