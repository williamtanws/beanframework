...
public class AddressSpecification extends CommonSpecification {
	public static <T> AbstractSpecification<T> getPageSpecification(DataTableRequest dataTableRequest) {
		return new AbstractSpecification<T>() {
			...
			public List<Selection<?>> toSelection(Root<T> root) {
				...
				multiselect.add(root.get(Address.UUID));
				multiselect.add(root.get(Address.ID));
				multiselect.add(root.get(Address.STREET_NAME));
				return multiselect;
			}
		};
	}
