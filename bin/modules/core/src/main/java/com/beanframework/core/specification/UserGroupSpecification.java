...
public class UserGroupSpecification extends CommonSpecification {
  public static <T> AbstractSpecification<T> getPageSpecification(
    ...
    return new AbstractSpecification<T>() {
      ...
      public List<Selection<?>> toSelection(Root<T> root) {
        ...
        multiselect.add(root.get(UserGroup.UUID));
        multiselect.add(root.get(UserGroup.ID));
        multiselect.add(root.get(UserGroup.NAME));
        return multiselect;
      }
    };
  }
