...
public class DynamicFieldSlotCsvEntityConverter implements EntityCsvConverter<DynamicFieldSlotCsv, DynamicFieldSlot> {

	protected static Logger LOGGER = LoggerFactory.getLogger(DynamicFieldSlotCsvEntityConverter.class);
	...
	private DynamicFieldSlot convertToEntity(DynamicFieldSlotCsv source, DynamicFieldSlot prototype) throws ConverterException {
		...
		try {
			...
			if (StringUtils.isNotBlank(source.getDynamicFieldId())) {
				...
				DynamicField entityDynamicField = modelService.findOneByProperties(parentProperties, DynamicField.class);
