...
public class UserPermissionSpecification extends CommonSpecification {
  public static <T> AbstractSpecification<T> getPageSpecification(
    ...
    return new AbstractSpecification<T>() {
      ...
      public List<Selection<?>> toSelection(Root<T> root) {
        ...
        multiselect.add(root.get(UserPermission.UUID));
        multiselect.add(root.get(UserPermission.ID));
        multiselect.add(root.get(UserPermission.NAME));
        multiselect.add(root.get(UserPermission.SORT));
        return multiselect;
      }
    };
  }
