...
public class RegionSpecification extends CommonSpecification {
	public static <T> AbstractSpecification<T> getPageSpecification(DataTableRequest dataTableRequest) {
		return new AbstractSpecification<T>() {
			...
			public List<Selection<?>> toSelection(Root<T> root) {
				...
				multiselect.add(root.get(Region.UUID));
				multiselect.add(root.get(Region.ID));
				multiselect.add(root.get(Region.NAME));
				multiselect.add(root.get(Region.ACTIVE));
				return multiselect;
			}
		};
	}
