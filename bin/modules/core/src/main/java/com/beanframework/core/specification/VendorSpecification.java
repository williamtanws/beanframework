...
public class VendorSpecification extends CommonSpecification {
	public static <T> AbstractSpecification<T> getPageSpecification(DataTableRequest dataTableRequest) {
		return new AbstractSpecification<T>() {
			...
			public List<Selection<?>> toSelection(Root<T> root) {
				...
				multiselect.add(root.get(Vendor.UUID));
				multiselect.add(root.get(Vendor.ID));
				multiselect.add(root.get(Vendor.NAME));
				return multiselect;
			}
		};
	}
