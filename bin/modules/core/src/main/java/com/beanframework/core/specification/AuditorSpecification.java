...
public class AuditorSpecification extends CommonSpecification {
  public static <T> AbstractSpecification<T> getPageSpecification(
    ...
    return new AbstractSpecification<T>() {
      ...
      public List<Selection<?>> toSelection(Root<T> root) {
        ...
        multiselect.add(root.get(Auditor.UUID));
        multiselect.add(root.get(Auditor.ID));
        multiselect.add(root.get(Auditor.NAME));
        return multiselect;
      }
    };
  }
