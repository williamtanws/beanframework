...
/**
 * Inbound File Adapter looks for files that match the given regular expression Any files that have
 * already been processed with the same name within the same JVM session will be ignored The poller
 * is transactional and will move the file to a processed directory on successful downstream
 * processing. If there is an exception in processing the file will be moved to a failed directory
 *
 */
@Configuration
public class CoreFilePollingIntegrationFlowConfig {
  ...
  public IntegrationFlow processedFileIntegration() {
    return IntegrationFlows.from(CoreIntegrationConfig.PROCESSED_CHANNEL)
        .handle(archiveFileProcessor).get();
  }

  @Bean
  public IntegrationFlow failedFileIntegration() {
    return IntegrationFlows.from(CoreIntegrationConfig.FAILED_CHANNEL).handle(archiveFileProcessor)
        .get();
  }
  ...
  public DirectoryScanner directoryScanner(@Value("${inbound.filename.regex}") String regex) {
    ...
    CompositeFileListFilter<File> filter =
        new CompositeFileListFilter<>(Arrays.asList(new AcceptAllFileListFilter<File>(),
            new IgnoreHiddenFileListFilter(), new RegexPatternFileListFilter(regex)));
