...
public class EmailEntityConverter implements EntityConverter<EmailDto, Email> {
  ...
  private Email convertToEntity(EmailDto source, Email prototype) throws ConverterException {
    ...
    try {
      ...
      if (StringUtils.equals(StringUtils.stripToNull(source.getToRecipients()),
          prototype.getToRecipients()) == Boolean.FALSE) {
        prototype.setToRecipients(StringUtils.stripToNull(source.getToRecipients()));
      }

      if (StringUtils.equals(StringUtils.stripToNull(source.getCcRecipients()),
          prototype.getCcRecipients()) == Boolean.FALSE) {
        prototype.setCcRecipients(StringUtils.stripToNull(source.getCcRecipients()));
      }

      if (StringUtils.equals(StringUtils.stripToNull(source.getBccRecipients()),
          prototype.getBccRecipients()) == Boolean.FALSE) {
        prototype.setBccRecipients(StringUtils.stripToNull(source.getBccRecipients()));
      }

      if (StringUtils.equals(StringUtils.stripToNull(source.getSubject()),
          prototype.getSubject()) == Boolean.FALSE) {
        prototype.setSubject(StringUtils.stripToNull(source.getSubject()));
      }

      if (StringUtils.equals(StringUtils.stripToNull(source.getText()),
          prototype.getText()) == Boolean.FALSE) {
      ...
      if (source.getSelectedMediaUuids() != null) {
        ...
        for (int i = 0; i < source.getSelectedMediaUuids().length; i++) {
          ...
          if (add) {
            Media enumeration = modelService
                .findOneByUuid(UUID.fromString(source.getSelectedMediaUuids()[i]), Media.class);
            if (enumeration != null) {
