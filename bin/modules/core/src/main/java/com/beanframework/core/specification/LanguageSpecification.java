...
public class LanguageSpecification extends CommonSpecification {
	public static <T> AbstractSpecification<T> getPageSpecification(DataTableRequest dataTableRequest) {
		return new AbstractSpecification<T>() {
			...
			public List<Selection<?>> toSelection(Root<T> root) {
				...
				multiselect.add(root.get(Language.UUID));
				multiselect.add(root.get(Language.ID));
				multiselect.add(root.get(Language.NAME));
				multiselect.add(root.get(Language.SORT));
				return multiselect;
			}
		};
	}
