...
public class UserAuthorityImportListener extends ImportListener {
  ...
  public void importer() {
    setType(CoreImportListenerConstants.UserAuthorityImport.TYPE);
    setName(CoreImportListenerConstants.UserAuthorityImport.NAME);
    setSort(CoreImportListenerConstants.UserAuthorityImport.SORT);
    setDescription(CoreImportListenerConstants.UserAuthorityImport.DESCRIPTION);
    setClassCsv(CoreImportListenerConstants.UserAuthorityImport.CLASS_CSV);
    setClassEntity(CoreImportListenerConstants.UserAuthorityImport.CLASS_ENTITY);
  ...
  public boolean save(UserAuthorityCsv csv) throws Exception {
    ...
    Map<String, List<UserAuthorityCsv>> userGroupMap =
        new HashMap<String, List<UserAuthorityCsv>>();
        ...
        for (int i = 0; i < userGroup.getUserAuthorities().size(); i++) {
          ...
          for (UserAuthorityCsv userAuthorityCsv : userGroupAuthorityList) {
            if (StringUtils.isNotBlank(userAuthorityCsv.getUserPermissionId())) {
              if (userGroup.getUserAuthorities().get(i).getUserPermission().getId()
                  .equals(userAuthorityCsv.getUserPermissionId())) {
                if (userGroup.getUserAuthorities().get(i).getUserRight().getId().equals("create")) {
                  if (StringUtils.isNotBlank(userAuthorityCsv.getCreate())) {
                    userGroup.getUserAuthorities().get(i)
                        .setEnabled(POSITIVE.equals(userAuthorityCsv.getCreate()) ? Boolean.TRUE
                            : Boolean.FALSE);
                  }
                }
                if (userGroup.getUserAuthorities().get(i).getUserRight().getId().equals("read")) {
                  if (StringUtils.isNotBlank(userAuthorityCsv.getRead())) {
                    userGroup.getUserAuthorities().get(i).setEnabled(
                        POSITIVE.equals(userAuthorityCsv.getRead()) ? Boolean.TRUE : Boolean.FALSE);
                  }
                }
                if (userGroup.getUserAuthorities().get(i).getUserRight().getId().equals("update")) {
                  if (StringUtils.isNotBlank(userAuthorityCsv.getUpdate())) {
                    userGroup.getUserAuthorities().get(i)
                        .setEnabled(POSITIVE.equals(userAuthorityCsv.getUpdate()) ? Boolean.TRUE
                            : Boolean.FALSE);
                  }
                }
                if (userGroup.getUserAuthorities().get(i).getUserRight().getId().equals("delete")) {
                  if (StringUtils.isNotBlank(userAuthorityCsv.getDelete())) {
                    userGroup.getUserAuthorities().get(i)
                        .setEnabled(POSITIVE.equals(userAuthorityCsv.getDelete()) ? Boolean.TRUE
                            : Boolean.FALSE);
                  }
                }
              }
