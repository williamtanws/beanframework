...
public class CompanySpecification extends CommonSpecification {
  public static <T> AbstractSpecification<T> getPageSpecification(
    ...
    return new AbstractSpecification<T>() {
      ...
      public List<Selection<?>> toSelection(Root<T> root) {
        ...
        multiselect.add(root.get(Company.UUID));
        multiselect.add(root.get(Company.ID));
        multiselect.add(root.get(Company.NAME));
        return multiselect;
      }
    };
  }
