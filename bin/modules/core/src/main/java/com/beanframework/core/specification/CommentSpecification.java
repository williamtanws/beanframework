  ...
  public static <T> Predicate getPredicate(DataTableRequest dataTableRequest, Root<T> root,
    ...
    if (dataTableRequest.isGlobalSearch() && StringUtils.isNotBlank(dataTableRequest.getSearch())) {
      if (isUuid(dataTableRequest.getSearch())) {
        predicates.add(
            cb.or(cb.equal(root.get(Comment.UUID), UUID.fromString(dataTableRequest.getSearch()))));
      }
      predicates.add(cb
          .or(cb.like(root.get(Comment.HTML), convertToLikePattern(dataTableRequest.getSearch()))));
      ...
      try {
        List<User> entities =
            modelService.findByPropertiesBySortByResult(properties, null, null, null, User.class);
      ...
      for (DataTableColumnSpecs specs : dataTableRequest.getColumns()) {
        if (StringUtils.isNotBlank(specs.getSearch())) {
          if (specs.getData().equals("html")) {
            predicates.add(
                cb.or(cb.like(root.get(Comment.HTML), convertToLikePattern(specs.getSearch()))));
          } else if (specs.getData().equals("user")) {
            ModelService modelService = SpringContext.getBean(ModelService.class);
            Map<String, Object> properties = new HashMap<String, Object>();
            properties.put(User.NAME, convertToLikePattern(specs.getSearch()));
            try {
              List<User> entities = modelService.findByPropertiesBySortByResult(properties, null,
                  null, null, User.class);
