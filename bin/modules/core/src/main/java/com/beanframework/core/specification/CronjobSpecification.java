...
public class CronjobSpecification extends CommonSpecification {
  public static <T> AbstractSpecification<T> getPageSpecification(
    ...
    return new AbstractSpecification<T>() {
      ...
      public List<Selection<?>> toSelection(Root<T> root) {
        ...
        multiselect.add(root.get(Cronjob.UUID));
        multiselect.add(root.get(Cronjob.ID));
        multiselect.add(root.get(Cronjob.NAME));
        return multiselect;
      }
    };
  }
