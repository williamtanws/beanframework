...
public class UserGroupPrepareInterceptor extends AbstractPrepareInterceptor<UserGroup> {
  ...
  public void onPrepare(UserGroup model, InterceptorContext context) throws InterceptorException {
    ...
    if (context.getOldModel() != null) {
      ...
      if (oldUserGroup.getId().equals(defaultAdminGroup)
          && model.getId().equals(defaultAdminGroup) == Boolean.FALSE
              | oldUserGroup.getId().equals(defaultEmployeeGroup)
          && model.getId().equals(defaultEmployeeGroup) == Boolean.FALSE
              | oldUserGroup.getId().equals(defaultCustomerGroup)
          && model.getId().equals(defaultCustomerGroup) == Boolean.FALSE
              | oldUserGroup.getId().equals(defaultVendorGroup)
          && model.getId().equals(defaultVendorGroup) == Boolean.FALSE) {
