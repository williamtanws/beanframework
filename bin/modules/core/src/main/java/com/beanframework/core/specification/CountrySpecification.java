...
public class CountrySpecification extends CommonSpecification {
	public static <T> AbstractSpecification<T> getPageSpecification(DataTableRequest dataTableRequest) {
		return new AbstractSpecification<T>() {
			...
			public List<Selection<?>> toSelection(Root<T> root) {
				...
				multiselect.add(root.get(Country.UUID));
				multiselect.add(root.get(Country.ID));
				multiselect.add(root.get(Country.NAME));
				multiselect.add(root.get(Country.ACTIVE));
				return multiselect;
			}
		};
	}
