...
public class ImexSpecification extends CommonSpecification {
	public static <T> AbstractSpecification<T> getPageSpecification(DataTableRequest dataTableRequest) {
		return new AbstractSpecification<T>() {
			...
			public List<Selection<?>> toSelection(Root<T> root) {
				...
				multiselect.add(root.get(Imex.UUID));
				multiselect.add(root.get(Imex.ID));
				multiselect.add(root.get(Imex.FILENAME));
				return multiselect;
			}
		};
	}
