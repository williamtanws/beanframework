...
public class EmployeeSpecification extends CommonSpecification {
  public static <T> AbstractSpecification<T> getPageSpecification(
    ...
    return new AbstractSpecification<T>() {
      ...
      public List<Selection<?>> toSelection(Root<T> root) {
        ...
        multiselect.add(root.get(Employee.UUID));
        multiselect.add(root.get(Employee.ID));
        multiselect.add(root.get(Employee.NAME));
        return multiselect;
      }
    };
  }
