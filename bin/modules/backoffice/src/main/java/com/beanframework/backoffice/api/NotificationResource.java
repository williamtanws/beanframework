...
public class NotificationResource extends AbstractResource {
  ...
  @RequestMapping(value = NotificationWebConstants.Path.Api.NOTIFICATION,
  ...
  @RequestMapping(value = NotificationWebConstants.Path.Api.NOTIFICATION_CHECKED,
      method = RequestMethod.GET, produces = "application/json")
  @ResponseBody
  ...
  @RequestMapping(value = NotificationWebConstants.Path.Api.NOTIFICATION_NEW,
      method = RequestMethod.GET, produces = "application/json")
  @ResponseBody
  public NotificationOverallDataTableResponseData newNotification(HttpServletRequest request)
      throws Exception {
    ...
    List<NotificationDto> result =
        notificationFacade.findAllNewNotificationByUser(currentUser.getUuid());
    NotificationOverallDataTableResponseData data = new NotificationOverallDataTableResponseData();

    if (result == null || result.isEmpty()) {
      String overallMessage = localeMessageService.getMessage("module.notification.overallmessage",
    ...
    for (String type : types) {
      NotificationMessageDataTableResponseData notificationMessage =
          getNotificationMessageByType(type, result);
    ...
    String overallMessage = localeMessageService.getMessage("module.notification.overallmessage",
        new String[] {String.valueOf(total), plural});
    data.setTotal(total);
    data.setOverallMessage(overallMessage);
  ...
  private int getTotalNotificationMessages(
      List<NotificationMessageDataTableResponseData> notificationMessages) {
    int count = 0;
  ...
  private NotificationMessageDataTableResponseData getNotificationMessageByType(String type,
      List<NotificationDto> result) {
    ...
    String plural = total > 1 ? "s" : "";
    String message = localeMessageService.getMessage(localemessagecode,
        new String[] {String.valueOf(total), plural});
