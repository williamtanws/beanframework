...
public class ImexResource extends AbstractResource {
  ...
  @PreAuthorize(ImexPreAuthorizeEnum.HAS_READ)
  @RequestMapping(value = ImexWebConstants.Path.Api.IMEX, method = RequestMethod.GET,
  ...
  @PreAuthorize(ImexPreAuthorizeEnum.HAS_READ)
  @RequestMapping(value = ImexWebConstants.Path.Api.IMEX_HISTORY, method = RequestMethod.GET,
      produces = "application/json")
  @ResponseBody
  public DataTableResponse<HistoryDataTableResponseData> history(HttpServletRequest request)
    ...
    return historyDataTableResponse(dataTableRequest, imexFacade.findHistory(dataTableRequest),
        imexFacade.countHistory(dataTableRequest));
  }
}
