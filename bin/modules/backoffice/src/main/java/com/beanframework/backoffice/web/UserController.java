...
public class UserController extends AbstractController {
  ...
  public RedirectView delete(
      @Valid @ModelAttribute(UserWebConstants.ModelAttribute.USER_DTO) UserDto dto, Model model,
      BindingResult bindingResult, @RequestParam Map<String, Object> requestParams,
      RedirectAttributes redirectAttributes) {
  ...
  public ResponseEntity<byte[]> getImage(@RequestParam Map<String, Object> requestParams)
    ...
    if (requestParams.get("uuid") != null
        && StringUtils.isNotBlank(requestParams.get("uuid").toString())) {
    ...
    if (StringUtils.isBlank(type)
        || (type.equals("original") == false && type.equals("thumbnail") == false)) {
      type = "thumbnail";
    }

    File picture = new File(MEDIA_LOCATION + File.separator + PROFILE_PICTURE_LOCATION,
        userUuid.toString() + File.separator + type + ".png");
    if (picture.exists()) {
  ...
  private ResponseEntity<byte[]> getDefaultPicture() throws BusinessException, IOException {
    ...
    String configurationValue =
        configurationFacade.get(CONFIGURATION_DEFAULT_AVATAR, "/static/core/img/avatar.png");
    ...
    if (configurationValue == null) {
      ...
      String mimeType =
          URLConnection.guessContentTypeFromName(FilenameUtils.getExtension(configurationValue));
