...
public class DeploymentController extends AbstractController {
	...
	public String page(@Valid @ModelAttribute(DeploymentWebConstants.ModelAttribute.DEPLOYMENT) Deployment deployment, Model model, @RequestParam Map<String, Object> requestParams) throws Exception {
		return VIEW_DEPLOYMENT;
	}

	@PreAuthorize(DeploymentPreAuthorizeEnum.HAS_READ)
	@GetMapping(value = DeploymentWebConstants.Path.DEPLOYMENT_FORM)
	public String form(@Valid @ModelAttribute(DeploymentWebConstants.ModelAttribute.DEPLOYMENT) Deployment deployment, Model model) throws Exception {
