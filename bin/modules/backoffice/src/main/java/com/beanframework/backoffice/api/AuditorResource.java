...
public class AuditorResource extends AbstractResource {
  ...
  @RequestMapping(value = AuditorWebConstants.Path.Api.AUDITOR, method = RequestMethod.GET,
  ...
  @PreAuthorize(PreAuthorizeEnum.HAS_READ)
  @RequestMapping(value = AuditorWebConstants.Path.Api.HISTORY, method = RequestMethod.GET,
      produces = "application/json")
  @ResponseBody
  public DataTableResponse<HistoryDataTableResponseData> history(HttpServletRequest request)
    ...
    return historyDataTableResponse(dataTableRequest, auditorFacade.findHistory(dataTableRequest),
        auditorFacade.countHistory(dataTableRequest));
  }
}
