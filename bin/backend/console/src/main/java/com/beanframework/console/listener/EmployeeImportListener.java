...
public class EmployeeImportListener extends ImportListener {
	...
	public void updateByContent(String content) throws Exception {
		List<EmployeeCsv> csvList = readCSVFile(new StringReader(content), EmployeeCsv.getUpdateProcessors());
		save(csvList);
	}
	...
	public List<EmployeeCsv> readCSVFile(Reader reader, CellProcessor[] processors) {
		...
		try {
			...
			EmployeeCsv csv;
			LOGGER.info("Start import " + ConsoleImportListenerConstants.EmployeeImport.NAME);
			while ((csv = beanReader.read(EmployeeCsv.class, header, processors)) != null) {
				LOGGER.info("lineNo={}, rowNo={}, {}", beanReader.getLineNumber(), beanReader.getRowNumber(), csv);
				csvList.add(csv);
			}
			LOGGER.info("Finished import " + ConsoleImportListenerConstants.EmployeeImport.NAME);
