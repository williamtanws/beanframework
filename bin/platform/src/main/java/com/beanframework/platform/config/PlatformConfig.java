...
@EnableJpaRepositories(basePackages = {"com.beanframework.*.repository"},
...
public class PlatformConfig {
  ...
  /*
   * Populate SpringBoot DataSourceProperties object directly from application.yml based on
   * prefix.Thanks to .yml, Hierachical data is mapped out of the box with matching-name properties
  ...
  private Properties jpaProperties() {
    ...
    properties.put("hibernate.c3p0.testConnectionOnCheckout",
        PLATFORM_HIBERNATE_C3P0_TESTCONNECTIONONCHECKOUT);

    properties.put("current_session_context_class", PLATFORM_CURRENT_SESSION_CONTEXT_CLASS);

    properties.put("hibernate.envers.autoRegisterListeners",
        PLATFORM_HIBERNATE_ENVERS_AUTOREGISTERLISTENERS);

    properties.put("hibernate.cache.use_second_level_cache",
        PLATFORM_HIBERNATE_CACHE_USE_SECOND_LEVEL_CACHE);
    properties.put("hibernate.cache.use_query_cache",
        PLATFORM_HIBERNATE_CACHE_USE_SECOND_LEVEL_CACHE);
    properties.put("hibernate.cache.region.factory_class",
        PLATFORM_HIBERNATE_CACHE_REGION_FACTORY_CLASS);
    properties.put("hibernate.cache.default_cache_concurrency_strategy",
        PLATFORM_HIBERNATE_DEFAULT_CACHE_CONCURRENCY_STRATEGRY);
    ...
    properties.put("hibernate.javax.cache.missing_cache_strategy",
        PLATFORM_HIBERNATE_JAVAX_CACHCE_MISSING_CACHE_STRATEGY);
    properties.put("hibernate.cache.auto_evict_collection_cache",
        PLATFORM_HIBERNATE_CACHE_AUTO_EVICT_COLLECTION_CACHE);

    properties.put("javax.persistence.sharedCache.mode",
        PLATFORM_JAVAX_PERSISTENCE_SHAREDCACHE_MODE);
